package net.biologeek.bot.plugin.serialization;

public enum ContentQueryType {

	extract, categories, categoryinfo, contributors, deletedrevisions, duplicatefiles, //
	extlinks, extracts, fileusage, flowinfo, globalusage, imageinfo, images, info, //
	iwlinks, langlinks, links, linkshere, mapdata, pageimages, pageprops, pageterms, //
	redirects, references, revisions, stashimageinfo, templates, transcludedin, transcodestatus, //
	videoinfo, wbentityusage, externallinks;
}
